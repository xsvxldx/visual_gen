...
- **`tail_dissolve` type (self-contained bridge):** on switch, crossfade the outgoing cue into
  A's own **last frame** (A-last, a single still fetched once — grab it at preload time so it's
  ready), then hard-cut to B. **Primary intent: keep A *playing* during the crossfade** — its
  live motion dissolves into its frozen last frame (`Blend(A_live, A_last, t, crossfade)` →
  `Single(B)`). One live stream, no B decoding during the window (B starts only at the cut, like
  a normal cut); it reuses the base-`crossfade` "pull a live frame each tick" path, just aimed at
  A-last instead of B. Fits the render seam with **no renderer changes**, but it is a *new type*,
  not one of the four base modes: it is a two-phase transition with its own engine orchestration
  (the `to` endpoint is A's own last frame, not B). Structurally it is the same multi-phase shape
  as morph (blend into a bridge, then hand off to B) with A-last standing in for the pre-rendered
  clip — a natural stepping stone to build right after the multi-phase orchestration lands,
  validating that shape before the clip machinery exists. (Variant, not chosen: freeze A-current
  at press — already held as `_last_frame` — and dissolve two held stills, so **zero** video
  decodes during the window; cheaper, but loses the live motion.)

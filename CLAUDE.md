...
- Transitions **tail_dissolve**: `docs/superpowers/specs/2026-07-11-tail-dissolve-design.md` — **DONE** (2026-07-12, 103 tests; operator manual pass pending before merge); 5th `TransitionMode`, live A crossfades into a still of its own last frame then hard-cuts to B, one decoder throughout. `t`-cycle order: cut → dip → crossfade → wipe → tail_dissolve → cut.
